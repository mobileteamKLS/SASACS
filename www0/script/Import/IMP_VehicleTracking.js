  ...
  function searchAWBNumber() {
    ...
    if ($("#txtBoE").val() == '') {
      ...
      return;
        ...
        //     text: "Please Wait..",
        // });
        ImportVehicleTracking('1', AirlinePrefix, AwbNumber, HawbNumber, CreatedByUserId, OrganizationBranchId, OrganizationId, BoENo,newdate);
    }
    $("#tblVehicleDetails").empty();
}
...
ImportVehicleTracking = function (OperationType, AirlinePrefix, AwbNumber, HawbNumber, CreatedByUserId, OrganizationBranchId, OrganizationId , BoENo,newdate) {
  ...
  $.ajax({
      type: 'POST',
      url: ACSServiceURL + "/ACS_Imp_VehicleTracking",
      data: JSON.stringify({ OperationType: OperationType, AirlinePrefix: "", AwbNumber: "", HawbNumber: HawbNumber, CreatedByUserId: CreatedByUserId, OrganizationBranchId: OrganizationBranchId, OrganizationId: OrganizationId, BoENumber: BoENo, BoEDate:newdate}),
...
FillControl = function (response) {
  ...
  if (obj.length > 0) {
      $.each(obj, function (i, d) {
        ...
        row += "<span id='spnEVTNo'>" + d.IVTNumber + "</span>";
        row += "</td>";
        ...
        row += "<i class='more-less zmdi zmdi-chevron-down' id='down-arrow' style='color:'#20A8D8' !important;font-size: 30px !important;' ></i>";
        row += "</a>";
        row += "</td>";
        row += "</tr>";
        row += "</tbody>";
