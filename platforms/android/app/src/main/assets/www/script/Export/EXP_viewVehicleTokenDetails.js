...
$(function () {
  ...
  getVTHousegeneratedDetails();
    ...
    function viewVehicleTokenDetails(){
        ...
        window.location.href = "EXP_viewHAWBDetails.html";
      }
      ...
      getVTgeneratedDetails = function(){
        ...
        success: function (response, xhr, textStatus) {
          ...
          console.log(response.d);
              ...
              var shimentDet = JSON.parse(shipmentDetails);
              localStorage.setItem('HouseObjectToUpdateVT', JSON.stringify(shimentDet));
              var obj = JSON.parse(VtDetails);
              console.log(VtDetails);
              console.log(obj);
            if (obj.length > 0) {
      ...
      cancelVT = function(){
        ...
        // var cancelVTXML = "<VTInfo><VehicleDet VTNO='" + VTno + "'/></VTInfo>"
          ...
          $.ajax({
          type: 'POST',
          url: ACSServiceURL + "/ACS_EXP_CancelVehicleToken",
          data: JSON.stringify({
          ...
          $(this).find('.vehicleDetails').each(function() {
            ...
            HouseObjectToUpdateVT.forEach((element, i) => {
              ...
              if(HouseObjectToUpdateVT.length > 1){
                nop = element.SBNOP;
                weight = element.SBGrossWeight;
              }else{
                nop = NoP;
                weight = GrWt;
              }
              vehicleXml +=  "<InsertSBData><SBData><rowIndex>" + (index) + "</rowIndex><AWBid>" + element.AWBID + "</AWBid><SBid>" + element.SBID + "</SBid><AllocateNOP>" + element.SBNOP + "</AllocateNOP><AllocateGrWt>" + element.SBGrossWeight  + "</AllocateGrWt><GHAID>" + GHAID + "</GHAID><VehicleNo>" + VehicleNo + "</VehicleNo><DriverName>" + DriverName + "</DriverName><DriverCountryCode></DriverCountryCode><DriverMobileNo>" + DriverMobNo + "</DriverMobileNo><AgentMobileNo>" + DriverMobNo + "</AgentMobileNo><DriverLicense>" + LicenseNo + "</DriverLicense><NoOfPieces>" + NoP + "</NoOfPieces><GrossWeight>" + GrWt + "</GrossWeight><Unit>KGS</Unit><VehicleToken>" + VTNO + "</VehicleToken><Remarks>" + GHANAME + "</Remarks><VehicleCargoType>" + CargoType + "</VehicleCargoType><CustodianID>" + GHAID + "</CustodianID></SBData></InsertSBData>";
          });
   ...
   if(r.test(v)) {
    ...
    $("#VTList").on("click", "#delete", function (ev) {
      ...
      $.ajax({
        type: 'POST',
        url: ACSServiceURL + "/ACS_EXP_Updates_VehicleToken",
        data: JSON.stringify({
