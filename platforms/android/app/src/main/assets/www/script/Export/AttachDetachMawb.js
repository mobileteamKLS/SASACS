...
$(function () {
  ...
  $("#btnDetach").attr('disabled', 'disabled');
  $("#btnAttach").css({ "background-color": "rgb(218 241 240)", "color": "#585a5d" , "border-color": "rgb(218 241 240)"});
  $("#btnDetach").css({ "background-color": "#f5bcbc", "color": "#585a5d" , "border-color": "#f5bcbc"});
...
function clickChk() {
  ...
  if (checkBox.checked == true){
    ...
    $("#btnAttach").css({ "background-color": "#03b6ae", "color": "white" , "border-color": "#03b6ae"});
    $("#btnDetach").removeAttr('disabled');
     $("#btnDetach").css({ "background-color": "red", "color": "white" , "border-color": "red"});
...
FillControl = function (response) {
  ...
  if (obj.length > 0) {
                ...
                $.each(obj, function (i, d) {
                  ...
                  row +="<i class='more-less zmdi zmdi-chevron-down' id='down-arrow' style='float: none !important;margin-right: 0px !important;font-size: 30px; color:#03b6ae;background-color: white;margin-left: 15px;'></i>";
                  row +="</button>";
                  row +="</div>";
...
$('table').on('click', '.btnAdd', function(){    
    var itemIndex = $(this).closest('tr').index();    var tr = "<tr style='border: 1px solid #aaaaaa;'><td><input type=text  class='fontSize12' style='text-align:left' id= 'hawb'></input</td><td><input type=image value = 'Add' src='img/icon-add.png' class=btnAdd></input></td></tr>";
    $(this).closest('table').append(tr);
    $(this).attr('src', 'img/icon-delete.png');
...
function onDelete(){
  ...
  document.getElementById("tblHawbInfo").deleteRow(rowIndex+1);
}
...
function Attach(){
  if($("#txtFlightPrefix").val() == ""){
    errmsg = "Alert</br>";
    errmsgcont = "AWB prefix is required.</br>";
    $.alert(errmsg,errmsgcont);
    return;
  }
  if($("#txtFlightNo").val() == ""){
    errmsg = "Alert</br>";
    errmsgcont = "MAWB No. is required.</br>";
    $.alert(errmsg,errmsgcont);
    return;
  }
  var textVal = "";
  data = [];
$('.fontSize12').each(function(){ 
    textVal = $(this).val(); 
    ...
    HawbNumbers = data.toString();
      console.log(data.toString());
      AttachDetachMawbHawb("1");
}
...
$('.fontSize12').each(function(){ 
    ...
    HawbNumbers = data.toString();
      console.log(data.toString());
      AttachDetachMawbHawb("2");
}

AttachDetachMawbHawb = function(OperationType){
  message = [];
  var AirlinePrefix = $("#txtFlightPrefix").val();
  var AwbNumber = $("#txtFlightNo").val();

  $.ajax({
  type: 'POST',
  url: ACSServiceURL + "/AttachDetachMawbhawb",
  data: JSON.stringify({
                "OperationType":OperationType,
                "AirlinePrefix":AirlinePrefix,
                "AwbNumber":AwbNumber,
                "HawbNumber":HawbNumbers,
  ...
  success: function (response, xhr, textStatus) {
      ...
      if (obj.length > 0) {
        obj.forEach((element, index, array) => {
          ...
          message.push(element.ErrorMessage + " <br>");
         
      });
      errmsg = "Alert</br>";
      errmsgcont = message.toString().replaceAll(",", "");
      $.alert(errmsg,errmsgcont);
      setTimeout(function () {
        window.location.href = 'dashboard-export.html'
    }, 4000);
