          ...
          if (obj.length > 0) {
            if (obj[0].ERRORMSG == undefined) {
              ...
              localStorage.setItem('GhaName', obj[0].GHAName);
                ...
                localStorage.setItem('DOLevel', DOLevel);
            ...
            if(obj[0].VTStatus == "Approved" || obj[0].VTStatus == "Generated" || obj[0].VTStatus == "Completed"){
...
console.log(dateandTime);
 ...
 var totalAmount= ($("#spTotalPayable").text()).substring(4); 
  ...
  if($("#txtBOEDate").val() == null || $("#txtBOEDate").val() == undefined){
    var _boEDate = "";
  }else{
    var _boEDate = $("#txtBOEDate").val();
  }
  if($("#txtBOENo").val() == null || $("#txtBOENo").val() == undefined){
    var _boENumber = "";
  }else{
...
$("#viewList").empty();
  ...
  getVTDetails = function(){
   ...
   if(text == "true"){
      if(VTStatus== "Pending" && BOEStatus== "Approved" ){

    
        FillMasterVTDetails();
    
      }else if(BOEStatus== "Pending" || BOEStatus== "Partial"){
        var disablePanel = true;  //or falsey value
        $("#accordionVT").on('hide.bs.collapse show.bs.collapse',  PanelEvent);
              
     }else if(VTStatus== "Partial" ){
     ...
     alert('Server not responding...');
      ...
      goToMasterDetailsAssignClearing = function (number) {
        ...
        if(DOLevel == "DOHAWBLevel")  {
          ...
          window.location.href = 'IMP_AssignClearingAgent.html';
 ...
 error: function (xhr, textStatus, errorThrown) {
     ...
     alert('Server not responding...');
      ...
      goToMasterBoEStatus = function (number) {
        ...
        //    return;  
        //  }else
          window.location.href = 'IMP_BoEStatus.html';
...
FillMasterVTDetails = function(){
     ...
     if (obj.length > 0) {
      var MAWBNo = obj[0].MAWBNo;
       if (obj[0].HAWBNo == null) {
        ...
        var hid = '"' + obj[0].HID + '"';
              row += " <div class= 'div-wrapper' style= 'background-color:#03b6ae !important'>";
              row += " <div onclick='goToMasterVTDetails(" + MAWBNo+ ", " + hid+ " )'>";
        ...
        var hid = '"' + obj[0].HID + '"';
             ...
             $.each(obj, function (i, d) {
              ...
              var hid = '"' + d.HID + '"';
              row += " <div class= 'div-wrapper' id='house' style= 'background-color:#03b6ae !important'>";
              row += " <div onclick='goToHouseVTDetails(" + house+ ", " + hid+ " )'>";
 ...
 error: function (xhr, textStatus, errorThrown) {
     ...
     alert('Server not responding...');
      ...
      goToMasterVTDetails = function (number,id) {
        console.log(number);
        localStorage.setItem('hawbNo', "");
        localStorage.setItem('HouseArrayData',id);
        localStorage.setItem('isHouse', false);
    ...
    goToHouseVTDetails =function (number, id) {
        localStorage.setItem('hawbNo', number);
        localStorage.setItem('HawbRowId', id);
        localStorage.setItem('isHouse', true);
        localStorage.setItem('HouseArrayData',id);
