...
function  GoToAttachDetachMawb(){
  localStorage.setItem('ImportsMenu','Attach/Detach MAWB/HAWB');
  window.location.href = "AttachDetachMawb.html";
}

function  GoToSubmitBoEASI(){
  localStorage.setItem('ImportsMenu','ASI Submission');
  window.location.href = "IMP_BoEandOoCDetails.html";
}
function  GoToDO(){
  localStorage.setItem('ImportsMenu','Generate DO');
  window.location.href = "IMP_BoEandOoCDetails.html";
}
function  GoToBOEStatus(){
  localStorage.setItem('ImportsMenu','BOE Status');
  window.location.href = "IMP_BoEandOoCDetails.html";
}
function  GoToClearingAgent(){
  localStorage.setItem('ImportsMenu','Assign Clearing Agent');
  window.location.href = "IMP_BoEandOoCDetails.html";
}
...
function searchAWBNumber() {
  if ($("#txtFlightPrefix").val() == '' || $("#txtFlightNo").val() == '' || $("#txtFlightPrefix").val().length != 3 || $("#txtFlightNo").val().length != 8) {
      ...
      ImportListingPage('1', AirlinePrefix, AwbNumber, CreatedByUserId, OrganizationBranchId, OrganizationId);
...
ImportListingPage = function (OperationType, AirlinePrefix, AwbNumber, CreatedByUserId, OrganizationBranchId, OrganizationId) {
  ...
  $.ajax({
      ...
      data: JSON.stringify({ OperationType: OperationType, AirlinePrefix: AirlinePrefix, AwbNumber: AwbNumber, HawbNumber: "", CreatedByUserId: CreatedByUserId, OrganizationBranchId: OrganizationBranchId, OrganizationId: OrganizationId }),
